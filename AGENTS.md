...
Plataforma local de treinamento de inglês para profissionais de DevOps, Cloud e SRE, falantes de português. Uso pessoal, um usuário, sem autenticação. O planejamento completo está em `docs/planejamento/` (plano geral, trilha de 5 níveis e 32 módulos, aula de exemplo). Estado atual: **MVP E0–E6 completo** — E5 entregue (30 aulas, 6 avaliações de módulo, avaliação de nível 1) e E6 mergeado (recomendação diária, drill por tag, checkpoint de 4 semanas, export/import JSON). **Nível 2 completo**: M07–M14 mergeados (40 aulas + 8 avaliações de módulo) + avaliação de nível 2 (`content/levels/level-2.yaml`: 30 itens cobrindo as 58 tags do nível, escrita dupla review+escalada, fala de refinamento de 90 s). Totais: 128 tags, 69 padrões BR, 70 aulas, 876 testes. A E4 (Claude API) está fora do roteiro por decisão do usuário (sem créditos de API): tudo segue por regras e heurísticas locais, sem LLM.
