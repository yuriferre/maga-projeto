...
export function RadarChart({ axes, size = 280 }: { axes: RadarAxis[]; size?: number }) {
  ...
  /** Rótulo à direita do gráfico ancora no início, à esquerda no fim; topo e base ficam centrados. */
  const anchor = (i: number) => { const c = Math.cos(angle(i)); return c > 0.15 ? "start" : c < -0.15 ? "end" : "middle"; };
  ...
  return (
    <div className="space-y-2">
      <svg viewBox={`0 0 ${size} ${size}`} overflow="visible" className="mx-auto block w-full max-w-xs" role="img" aria-label="Radar de competências">
        ...
        {axes.map((a, i) => {
          ...
          const l = point(i, 1.08);
          return (
            <g key={a.key}>
              <circle cx={p.x} cy={p.y} r={3} fill={a.value === null ? "#cbd5e1" : "#4f46e5"} />
              <text x={l.x} y={l.y} textAnchor={anchor(i)} dominantBaseline="middle" className="fill-slate-600" fontSize={11}>
