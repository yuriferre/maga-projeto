...
import { ApiError, type PlacementAssessment, type PlacementState, type ReadAloudEntry, type WritingFeedback, type SpeakingMetrics as SpeakingResult } from "../../lib/api.ts";
import { placement } from "../../lib/content.ts";
import { Button } from "../ui/Button.tsx";
import { Markdown } from "../ui/Markdown.tsx";
import { Stepper } from "../lesson/Stepper.tsx";
import { ExerciseList } from "../exercises/ExerciseList.tsx";
import { Listening } from "../lesson/Listening.tsx";
import { Writing } from "../lesson/Writing.tsx";
import { Speaking } from "../lesson/Speaking.tsx";
...
type FlowApi = {
  state(): Promise<PlacementState>;
  submitWriting(body: { text: string; selfScore?: number }): Promise<{ id: number; feedback: WritingFeedback }>;
  submitSpeaking(body: { readAloud: ReadAloudEntry[]; transcript: string; durationSec: number; selfConfidence?: number }): Promise<{ id: number; metrics: SpeakingResult }>;
  finish(): Promise<{ assessment: PlacementAssessment }>;
};

/** Fluxo do teste inicial, reutilizado pelo checkpoint de 4 semanas (mesmo formato). */
export function PlacementFlow({ refId, title, flowApi }: { refId: "placement" | "checkpoint"; title: string; flowApi: FlowApi }) {
  ...
  const finish = async () => {
    ...
    try {
      const res = await flowApi.finish();
  ...
  const backToBlocks = () => {
    flowApi.state().then((s) => { setState(s); setPassageIndex(0); setScriptIndex(0); setMissing(null); setStage(resumeStage(s)); }).catch((e: Error) => setError(e.message));
  };
  ...
  const body = (() => {
    switch (stage) {
      ...
      case "reading": {
        ...
        return (
          <div className="space-y-4">
            ...
            <ExerciseList key={passage.id} lessonId={refId} block="placement" exercises={passage.questions} feedback="deferred" initialAnswered={answered}
      ...
      case "grammar": {
        ...
        return (
          <div className="space-y-4">
            ...
            <ExerciseList key={stage} lessonId={refId} block="placement" exercises={block.questions} feedback="deferred" initialAnswered={answered} onFinished={() => setStage(next(stage))} />
          </div>
        );
      }
      case "listening": {
        ...
        return (
          <div className="space-y-4">
            ...
            <Listening key={script.id} lessonId={refId} block="placement" lines={script.lines} questions={script.questions} feedback="deferred" initialAnswered={answered}
      ...
      case "writing":
        return (
          <div className="space-y-4">
            <Writing spec={placement.writing} fetchLatest={() => flowApi.state().then((s) => s.run.writing)} submit={(b) => flowApi.submitWriting(b)} onSaved={setWritingScore} />
        ...
        return (
          <div className="space-y-6">
            ...
            <Speaking spec={placement.speaking.modeA} title="Pergunta" onSubmitted={setSpoken}
              submit={(b) => flowApi.submitSpeaking({ ...b, readAloud: readAloud.filter((r) => r.transcript.trim().length > 0) })} />
